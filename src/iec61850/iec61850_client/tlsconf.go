...
func buildTLS(conn *Iec61850Connection) (*tls.Config, error) {
	...
	if len(pinned) > 0 || conn.ChainValidation {
		...
		cfg.VerifyPeerCertificate = func(rawCerts [][]byte, _ [][]*x509.Certificate) error {
			...
			if len(pinned) > 0 {
				...
				if !ok {
					return fmt.Errorf("peer certificate is not in the allowed list")
				}
			}
			if chainValidation {
	...
	Log(LogLevelBasic, "%s TLS versions %s..%s", conn.Name, tlsVersionName(minV), tlsVersionName(maxV))
	if (conn.AllowTLSv10 || conn.AllowTLSv11) && minV >= tls.VersionTLS12 {
		Log(LogLevelBasic, "%s TLS 1.0/1.1 requested but not available in this build; minimum is %s",
			conn.Name, tlsVersionName(minV))
	}
...
// lowest and highest version enabled. The C# cascade could produce a
// minimum above its maximum for some flag combinations (deviation D5).
func tlsVersionWindow(conn *Iec61850Connection) (uint16, uint16) {
	var versions []uint16
	if conn.AllowTLSv10 {
	...
	// Go no longer negotiates TLS 1.0/1.1 by default.
