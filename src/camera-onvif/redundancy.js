...
function Start(interval, clientMongo, db, configObj, MongoStatus) {
  ProcessRedundancy(clientMongo, db, configObj, MongoStatus)
...
async function ProcessRedundancy(clientMongo, db, configObj, MongoStatus) {
  if (!clientMongo || !db) return
  if (MongoStatus && !MongoStatus.HintMongoIsConnected) {
    ProcessActive = false
    return
  }
  ...
  try {
    ...
    if (instance?.activeNodeName === configObj.nodeName) {
      ...
      ProcessRedundancy.lastActiveNodeKeepAliveTimeTag = instKeepAliveTimeTag
      if (
        ProcessRedundancy.countKeepAliveNotUpdated > countKeepAliveUpdatesLimit
      ) {
    ...
    if (ProcessActive) {
      ...
      db.collection(configObj.ProtocolDriverInstancesCollectionName).updateOne(
        {
          protocolDriver: AppDefs.NAME,
          protocolDriverInstanceNumber: new Double(configObj.Instance),
        },
